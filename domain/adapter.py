...
class Adapter:
    ...
    def update(self, model):
        ...
        health = (self.food[-1] + self.health[-1]) / self.max_health[-1]

        self.K_monster_quantity.append(1 + (level_reached / 20) + (fight_success * 0.3))
        self.K_monster_strength.append(1 + (level_reached / 15) + (health * 0.3) + (strength / 20) + (dexterity / 20))
        self.K_items_quantity.append(1 - (progress * 0.2))
        self.K_items_power.append(1 + ((hits_taken - hits_dealt) / (hits_taken + hits_dealt + 1)))
